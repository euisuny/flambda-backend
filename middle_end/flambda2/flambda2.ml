...
let dump_validator_files filename src_init src_normalized res_init
    res_normalized =
  Misc.protect_writing_to_file ~filename ~f:(fun out ->
      let ppf = Format.formatter_of_out_channel out in
      Format.fprintf ppf
        "\n\n\n------------------ Translated Original ------------------\n\n";
      Flambda2_core.print ppf src_init;
      Format.fprintf ppf
        "\n\n\n------------------ Normalized Original ------------------\n\n";
      Flambda2_core.print ppf src_normalized;
      Format.fprintf ppf
        "\n\n\n------------------ Translated Result ------------------\n\n";
      Flambda2_core.print ppf res_init;
      Format.fprintf ppf
        "\n\n\n------------------ Normalized Result ------------------\n\n";
      Flambda2_core.print ppf res_normalized;
      Format.fprintf ppf "\n%!")

let validate filename (src : Flambda_unit.t) (res : Flambda_unit.t) =
  let src_core =
    Profile.record ~accumulate:true "translate_src"
      Translate.flambda_unit_to_core src
  in
  let src_core' =
    Profile.record ~accumulate:true "normalize_src" Normalize.normalize src_core
  in
  let res_core =
    Profile.record ~accumulate:true "translate_res"
      Translate.flambda_unit_to_core res
  in
  let res_core' =
    ...
    let flambda, offsets, cmx, all_code =
      ...
      | Normal, Normal ->
        ...
        then (
          ...
          with _ -> Format.eprintf "fλ2: %s FAIL [ERROR]@." filename);
        (if Flambda_features.inlining_report ()
        then
