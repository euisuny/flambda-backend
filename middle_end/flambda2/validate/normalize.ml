...
(* [Set of closures]
   ...
   i.e. it is the code generated by
    ...
    to the code *)
and subst_bound_set_of_closures (bound : Bound_var.t)
    ~(let_body : static_const_or_code) (n : named) (e : core_exp) =
  match n with
  | Literal (Simple v) ->
    (match let_body with
     | Static_const const ->
...
let rec subst_cont (cont_e1: core_exp) (k: Bound_continuation.t)
  ...
  | Named (Literal (Simple _ | Slot _ | Res_cont Never_returns | Code_id _)
          ...
          | Rec_info _) -> cont_e1
  | Let _ ->
    let_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  | Let_cont _ ->
    let_cont_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  | Apply _ ->
    apply_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  ...
  | Lambda _ ->
    lambda_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  | Handler _ ->
    handler_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  | Switch _ ->
    switch_fix (fun e -> subst_cont e k args cont_e2) cont_e1
  | Invalid _ -> cont_e1
        ...
        let exp =
          core_fmap
            (fun _ v ->
               ...
               | None ->
                 (match must_be_simple v with
                  | Some s -> if (Simple.same (Simple.var bound.my_region) s)
                              then region else v
                  | None -> v)
            ) () exp
        in
...
and subst_my_closure_body phi (clo: set_of_closures) (e : core_exp) : core_exp =
  ...
  | Let _ -> let_fix (subst_my_closure_body phi clo) e
  | Let_cont _ -> let_cont_fix (subst_my_closure_body phi clo) e
  | Apply _ -> apply_fix (subst_my_closure_body phi clo) e
  | Apply_cont _ -> apply_cont_fix (subst_my_closure_body phi clo) e
  | Lambda _ -> lambda_fix (subst_my_closure_body phi clo) e
  | Handler _ -> handler_fix (subst_my_closure_body phi clo) e
  | Switch _ -> switch_fix (subst_my_closure_body phi clo) e
  | Invalid _ -> e
...
and step_named (e : core_exp) : core_exp =
  let body =
    match must_be_named e with
    | Some n -> n
    | None -> Misc.fatal_error "[step named] Expected name expr"
  in
  match body with
  ...
  | Static_consts _ -> static_const_group_fix step e
...
let rec inline_handlers (e : core_exp) =
  match Expr.descr e with
  | Named _ ->
    named_fix inline_handlers (fun () x -> x) () e
  | Let _ -> let_fix inline_handlers e
  | Let_cont e -> inline_let_cont e
  | Apply _ -> apply_fix inline_handlers e
  | Apply_cont _ -> apply_cont_fix inline_handlers e
  | Lambda _ -> lambda_fix inline_handlers e
  | Handler _ -> handler_fix inline_handlers e
  | Switch _ -> switch_fix inline_handlers e
  | Invalid _ -> e
