...
and subst_pattern_static
  ...
  | Named named ->
    (match bound with
     ...
     | Set_of_closures _ ->
       Misc.fatal_error "Should not be reached"
...
let rec step (c: clo) (e: core_exp) : clo * core_exp =
  match Expr.descr e with
  | Let { let_abst; expr_body } ->
...
and step_let c let_abst body : clo * core_exp =
...
and step_let_cont c ({handler; body}:let_cont_expr) : clo * core_exp =
  Core_continuation_handler.pattern_match handler
    (fun args e2 ->
        Core_letcont_body.pattern_match body
          (fun k e1 ->
...
and step_apply_function_decls c phi slot function_decls
      callee continuation exn_continuation region apply_args : clo * core_exp =
  match Function_slot.Lmap.find_opt slot function_decls |>
        Option.map Expr.descr with
  | Some (Lambda exp) ->
...
and step_apply_cont c k args : clo * core_exp =
  (* [ApplyCont]
            args ⟶ args'
      --------------------------
     k args ⟶ k args'       *)
...
and step_named c (body : named) : clo * core_exp =
  match body with
...
(* This is a "normalization" of [named] expression, in quotations because there
  is some simple evaluation that occurs for primitive arithmetic expressions *)
and step_named_for_let (c : clo) (var: Bound_for_let.t) (body: named)
  : Bound_for_let.t * clo *  core_exp =
  match body with
  | Literal _ (* A [Simple] is a register-sized value *)
  | Rec_info _ (* Information about inlining recursive calls, an integer variable *) ->
  ...
  core_fmap
    (fun () e ->
       ...
       | Slot (var, Function_slot slot) ->
         Expr.create_named (Closure_expr (var, slot, Clo.find var clo))
       | (Slot (_, Value_slot _) | Cont _ | Res_cont _ | Code_id _ | Simple _) ->
