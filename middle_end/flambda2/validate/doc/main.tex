...
\begin{mathpar}
  ...
  \inferrule
      [\footnotesize LetCloNamed-$\beta$]
      {x = \mathcal{K}[i]}
      {\letexp{(\mathsf{clo} \, \Phi\, \; \mathcal{K})}{P}{e} \step \subst{e}{x}{(\Phi, \pi_1 \; P[i], P)}}
  ...
  \inferrule
      [\footnotesize LetStaticCloNamed-$\beta$]
      {x \in \mathcal{K}^{\uparrow}}
      {\letexp{(\mathsf{clo}^{\uparrow} \, \Phi \, \; \mathcal{K}^{\uparrow})}{P}{e} \step \subst{e}{x}{(\Phi, x, P)}}
  ...
  \inferrule
      ...
      \letcloexp{(\mathbf{f}; \mathcal{K})}{([f_{\mathsf{fn}} \mapsto
          \subst{\abst{\rho,\,\, res_k,\,exn_k,\,\args{\expr}}{e_1}}
                {\pi^\rho_{\mathsf{fn} (f_{\mathsf{fn}} \rightarrow g_{\mathsf{fn}})}}
                {g_{\mathsf{fn}}}
        ] ; P)}{e_2}\\

  \inferrule
      [\footnotesize Closure-$\beta$]
      {\Phi \; \mathsf{fresh}}
      {\letcloexp{(\mathbf{f}; \mathcal{K})}{([f_{\mathsf{fn}} \mapsto \abst{\rho, \, res_k,\,exn_k,\,\args{\expr}}{e_1}] ; P)}{e_2}
        \step
      }\vspace{-1em}\\
      \letcloexp{\Phi \; (\mathbf{f}; \mathcal{K})}{([f_{\mathsf{fn}} \mapsto
...
\end{mathpar}
\caption{Evaluation Rules for Flambda2 Core (Continued.)}
\label{fig:reduction}
\end{figure}
