...
\begin{mathpar}
  ...
  \inferrule
      ...
      {\letexp{(\mathsf{clo} \; \mathcal{K}^{\uparrow})}{P}{e} \step \subst{e}{x}{(x, P)}}
  ...
  \inferrule
      ...
      {}
      {\letcloexp{(v_{\mathsf{val}} ; \mathcal{K})}{(\idexpr_v; P)}{e} \step
        \letcloexp{\mathcal{K}}{\subst{P}{\pi^\rho_{\mathsf{val(v)}}}{\idexpr_v}}{e}}

 \inferrule
     [ClosureFn]
     {[g_{\mathsf{fn}} \mapsto \mathbf{g}] \in K \\ (g_{\mathsf{fn}}, \expr_g) \in P}
     {\letcloexp{([f_{\mathsf{fn}} \mapsto \mathbf{f}]; \mathcal{K})}{([f_{\mathsf{fn}} \mapsto \abst{\rho, \, res_k,\,exn_k,\,\args{\expr}}{e_1}] ; P)}{e_2}
       \step
     }\vspace{-1em}\\
     \letcloexp{([f_{\mathsf{fn}} \mapsto \mathbf{f}]; \mathcal{K})}{([f_{\mathsf{fn}} \mapsto
         \subst{\abst{\, res_k,\,exn_k,\,\args{\expr}}{e_1}}
               {\pi^\rho_{\mathsf{fn} (f \rightarrow g)}}
               {\mathbf{g}}
       ] ; P)}{e_2}\\
  ...
  \inferrule
      ...
      {}
      {\apply{\direct \; (\lambda P \; k_r \; k_e \; x.e)}{P'}{res_k}{exn_k}{\args{v}}
       \step
       \subst{e}{P}{P'}\substchain{k_r}{res_k}\substchain{k_e}{exn_k}\substchain{x}{\args{v}}
      }
